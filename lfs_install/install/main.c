...
int main(int argc,char **argv)
{
	...
	copy_file("build/boot/vmlinuz","build/mnt/vmlinuz");
	copy_file("build/boot/initramfs","build/mnt/initramfs");
	copy_file("boot.conf","build/mnt/boot.conf");
	while(umount("build/mnt"));
	...
	read_id("build/root/rootid");
	lseek(loopfd,0,0);
	write(loopfd,bootid,16);


	format_loop("build/root/bin/mkfs.ext3");
	if(mount(loop_name,"build/mnt","ext3",0,0))
	{
		loop_fini();
		return 1;
	}
	copy_file("build/system.cpio.xz","build/mnt/system.cpio.xz");
	copy_file("vmlinuz","build/mnt/vmlinuz");
	copy_file("boot.conf.product","build/mnt/boot.conf");
	copy_file("build/boot/bootx64.efi","build/mnt/bootx64.efi");
	while(umount("build/mnt"));
