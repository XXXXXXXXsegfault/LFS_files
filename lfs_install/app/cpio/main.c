/*
 * NOTE: This implementation has no support for special files other than regular files, directories and symbolic links.
 * */
	...
	while(dir=readdir(&db))
	{
		if(strcmp(dir->name,".")&&strcmp(dir->name,".."))
		{
			...
			else if(type==STAT_LNK)
			{
				...
				munmap(new_name,l);
				if((l=readlinkatl(dirfd,dir->name,fbuf,4096))<0)
				{
					exit(1);
				}
				write(fd,fbuf,l);
...
int cpio_unpack2(int fdi,char *dst)
{
	int dir,fdo;
	static char buf[4100];
	struct cpio header;
	...
	while(sock_read(fdi,&header,sizeof(header))==sizeof(header))
	{
		if(memcmp(header.magic,"070701",6))
		{
			return 1;
		...
		else if((mode&0170000)==STAT_LNK)
		{
			...
			if(sock_read(fdi,buf,size)!=size)
			{
				return 1;
			}
			buf[size]=0;
		...
		else if((mode&0170000)==STAT_REG)
		{
			...
			while(1)
			{
				if(size>4096)
				{
					if(sock_read(fdi,buf,4096)!=4096)
					{
						return 1;
					}
					write(fdo,buf,4096);
