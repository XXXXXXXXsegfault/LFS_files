...
int fill_e820(struct linux_boot_params *boot_params)
{
	...
	while(x1<nentries)
	{
		...
		if(x&&boot_params->e820_table[x-1].addr+boot_params->e820_table[x-1].size
			==boot_params->e820_table[x].addr
			&&boot_params->e820_table[x-1].type==boot_params->e820_table[x].type)
		{
			boot_params->e820_table[x-1].size+=boot_params->e820_table[x].size;
			--x;
			--boot_params->n_e820;
		}
		++boot_params->n_e820;
...
char boot_cmdline[4096];
void do_boot(void *image,void *boot_params);
int boot_init(char *cmdline,struct ext2_image *kernel,struct ext2_image *initramfs)
{
	...
	strcat(boot_cmdline," acpi_rsdp=0x");
	...
	boot_params->sinfo.base=(unsigned long long)fbstart;
	boot_params->sinfo.ext_base=(unsigned long long)fbstart>>32;
	...
	if(initramfs->size)
	{
		boot_params->ramdisk_image=(unsigned long)linux_buf+0x8000000-initramfs->size;
		boot_params->ext_ramdisk_image=(unsigned long)linux_buf+0x8000000-initramfs->size>>32;
	}
	boot_params->cmdline=(unsigned long)boot_cmdline;
	boot_params->ext_cmdline=(unsigned long)boot_cmdline>>32;
	if(initramfs->size)
	{
		memcpy(linux_buf+0x8000000-initramfs->size,initramfs->data,initramfs->size);
	}
